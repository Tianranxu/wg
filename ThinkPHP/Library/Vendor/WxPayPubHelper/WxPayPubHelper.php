...
{
    public $WxPayConf_pub;
	function __construct($appId, $mchId, $key) {
	    $this->WxPayConf_pub=new WxPayConf_pub($appId, $mchId, $key);
	}
	...
	public function getSign($Obj)
	{
		...
		//签名步骤二：在string后加入KEY
		$String = $String."&key=".$this->WxPayConf_pub->key;
	...
	function postXmlSSLCurl($xml,$url,$second=30)
	{
		...
		curl_setopt($ch,CURLOPT_SSLCERT, $this->WxPayConf_pub->sslCertPath);
		//默认格式为PEM，可以注释
		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
		curl_setopt($ch,CURLOPT_SSLKEY, $this->WxPayConf_pub->sslKeyPath);
		//post提交方式
		curl_setopt($ch,CURLOPT_POST, true);
		curl_setopt($ch,CURLOPT_POSTFIELDS,$xml);
		$data = curl_exec($ch);
...
class Wxpay_client_pub extends Common_util_pub 
{
	...
	function createXml()
	{
	   	$this->parameters["appid"] = $this->WxPayConf_pub->appId;//公众账号ID
	   	$this->parameters["mch_id"] = $this->WxPayConf_pub->mchId;//商户号
...
{	
	function __construct($appId, $mchId, $key) 
	{
		...
		//设置curl超时时间
		$this->curl_timeout = $this->WxPayConf_pub->curlTimeout;
	}
...
class NativeCall_pub extends Wxpay_server_pub
{
	...
	function createXml()
	{
		if($this->returnParameters["return_code"] == "SUCCESS"){
		   	$this->returnParameters["appid"] = $this->WxPayConf_pub->appId;//公众账号ID
		   	$this->returnParameters["mch_id"] = $this->WxPayConf_pub->mchId;//商户号
...
class JsApi_pub extends Common_util_pub
{
	...
	{
		$urlObj["appid"] = $this->WxPayConf_pub->appId;
	...
	{
		$urlObj["appid"] = $this->WxPayConf_pub->appId;
		$urlObj["secret"] = $this->WxPayConf_pub->appSecret;
	...
	{
		$jsApiObj["appId"] = $this->WxPayConf_pub->appId;
		$timeStamp = time();
